...
//end of nav
const myAdLink = 'https://ey43.com/4/6838015';
...
karentPage();
function karentPage(){
    fetch(`${latestAPI}${saGitna}${myAPI}&page=${currentCount}`)
     .then(res => res.json())
     .then(sunod => {
         //console.log(sunod);
                  moviePosters.forEach((images, index)=>{
...
function searchFetch(){
    ...
    .then(search => {
     ...
     rate.textContent = 'N/A';
        ...
        //to navigate page
    //chexk ung total pages

    /*nextBtn.addEventListener('click', ()=> {
    //console.log(search.total_pages)
    if(resultPage >= search.total_pages){
        moviePosters.forEach((index, images) => {
            images.classList.add('skeleton');
        });
    }else{
        window.scrollTo({top: 0, behavior: 'smooth'});
