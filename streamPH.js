function checkHash() {
  if (window.location.hash !== '') {
    ...
    // Find the movie with the matching ID and trigger the modal window
    fetch(`${popularAPI}${saGitna}${myAPI}`)
      ...
      .then(data => {
        ...
        serverBtns(data, index);
        updateMetaTag('description', `${data.results[index].overview}`);
updateOgTag('og:image', `${originalImage}${movie.backdrop_path}`);
      });
  }
}
